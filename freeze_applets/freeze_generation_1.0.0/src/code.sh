...
main ()
{
    ...
    bcftools -h "$sample.$i.pVCF.vcf.gz" | tail -1 > pvcf_header.txt
    dx download "${eve}" -o eve.vcf.gz
    dx download "${blacklist}" -o blacklist.txt
    eve_qd2fail eve.vcf.gz > eve.qd2failed_samples.txt
    subset_eve_qd2fail.pl eve.qd2failed_samples.txt GHS > ${sample}.qd2failed_samples.txt
    rm eve.qd2failed_samples.txt &
    qdfilter_consolidation.pl pvcf_header.txt ${sample}.qd2failed_samples.txt blacklist.txt | bgzip > ${sample}.qd2failed_indices.txt
    rm ${sample}.qd2failed_samples.txt &
