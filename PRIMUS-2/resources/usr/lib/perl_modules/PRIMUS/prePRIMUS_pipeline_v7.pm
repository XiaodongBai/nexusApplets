...
sub run_prePRIMUS_main
{
	...
	if($INTERNAL_REF)
	{
		...
		my($AIMs_file,@AIMs) = get_AIMs($unrelated_stem) if $remove_AIMs == 1;
	...
	elsif($ALTERNATIVE_REF)
	{
		...
		## Use unrelated samples
		my ($unrelated_stem, @unrelated_samples) = get_unrelateds($autosomal_no_dup_stem);
		my($merged_stem,$flipped_SNP_arr_ref,$remove_SNP_arr_ref) = merge($unrelated_stem,$alt_ref_stem);
	...
	else ## Use HapMap3 as a reference
	{
		...
		{
			($merged_stem,$flipped_SNP_arr_ref,$remove_SNP_arr_ref) = merge($unrelated_stem,$HM3_STEM);
			my $PCA_plot = make_PCA_plot($merged_stem,$HM3_STEM,$study_name,"hapmap3") if !$no_PCA_plot;
			if($no_automatic_IBD){die "--no_automatic_IBD was selected. Please see $PCA_plot to select a reference population and rerun\n";}
			@ref_pops = pick_reference_populations($merged_stem,$study_name);
		}
		

		#### Proceeds down one of 4 paths depending on whether it is admixed and the number sample unrelated samples
		my $allele_freqs;
		...
		if((is_admixed(@ref_pops) || $remove_AIMs ) && @unrelated_samples >= $MIN_SAMPLES_WITHOUT_REF && !$keep_AIMs)
		{
			...
			print $LOG "\nREMOVING AIMS and >= $MIN_SAMPLES_WITHOUT_REF UNRELATED SAMPLES\n" if $verbose > 0;
			my $PCA_plot = make_PCA_plot($unrelated_stem,"",$study_name) if !$no_PCA_plot;
			my($AIMs_file,@AIMs) = get_AIMs($unrelated_stem);
...
sub get_MT_estimates
{
	...
	foreach my $name1 (keys %data)
	{
		foreach my $name2 (keys %data)
		{
			...
			$MT_estimates{$name1}{$name2} = $match;
			$MT_estimates{$name2}{$name1} = $match;
...
sub pick_reference_populations
{
	my $data_stem = shift;
	my $study_name = shift;
	my $rerun_smartpca= shift;

	print "\nSelecting reference population for $data_stem\n" if $verbose > 0;
	print $LOG "\nSelecting reference population for $data_stem\n" if $verbose > 0;
	$study_name = get_file_name_from_stem($data_stem) if $study_name eq "";
	
	run_smartpca($data_stem) if (!-e "$data_stem.evec" || $rerun_smartpca == 1);
	
	my %unrelated_samples;
	my %KDEs;
	my @PC1;
	my @PC2;
	my %ref_pops;

	## Make a three column table of the HM3 samples: POP PCV1 PVC2
	...
	foreach my $pop (@hm3_pops)
	{
		...
		while(my $line = <IN>)
		{
			...
			$unrelated_samples{$IID} = $pop;
		}
		close(IN);
	}
	
	# Process evec file
	open(IN,"$data_stem.evec") or die "can't open $data_stem.evec: $!\n";
	my $header = <IN>;
	open(OUT,">$data_stem\_PCV_vals.txt") or die "can't open $data_stem\_PCV_vals.txt: $!\n";
	while(my $line = <IN>)
	{
		$line =~ s/^\s+//; ## get rid of the leading white space
		my ($name,$pc1,$pc2,@rest) = split(/\s+/,$line);
		my ($FID,$IID) = split(/:/,$name);
		if(exists $unrelated_samples{$IID})
		{
			print OUT "$unrelated_samples{$IID}\t$pc1\t$pc2\n";
		}
	}
	close(IN);
	close(OUT);
...
## INPUT2: the .evec file generated from the stem_name data (optional)
## INPUT3: You can force a run of smartpca even if the .evec file already exists (optional)
...
sub get_AIMs
{
	...
	$evec_file = "$stem_name.evec" if $evec_file eq "";
	
	print "\nGetting AIMs for $stem_name\n" if $verbose > 0;
	print $LOG "\nGetting AIMs for $stem_name\n" if $verbose > 0;

	## run_smartpca (e.g. EIGENSTRAT)
	if(!-e $evec_file || $rerun_smartpca == 1)
	{
		run_smartpca($stem_name);
	}

	## Convert .evec file into format for PLINK for association tests
	open(IN,"$evec_file") or die "Can't open $evec_file; $!\n";
	open(OUT,">$evec_file\_temp") or die "Can't write to $evec_file\_temp; $!\n";
	my $header = <IN>;
	my %pops;
	while(my $line = <IN>)
	{
		$line =~ s/^\s+//;
		my ($name,@pcv) = split(/\s+/,$line);
		my ($fid,$iid) = split(/:/,$name);
		print OUT "$fid $iid @pcv\n"; 
	}
	close(IN);
	close(OUT);
	$intermediate_files{"$evec_file\_temp"} = 1;

	## Run Association test for eavh PCV
	system("$PLINK --noweb --bfile $stem_name --pheno $evec_file\_temp --allow-no-sex --mpheno 1 --assoc $plink_silent --out $stem_name\_PCV1");
	$intermediate_files{"$stem_name\_PCV1.log"} = 1;
	$intermediate_files{"$stem_name\_PCV1.qassoc"} = 1;
	system("$PLINK --noweb --bfile $stem_name --pheno $evec_file\_temp --allow-no-sex --mpheno 2 --assoc $plink_silent --out $stem_name\_PCV2");
...
## Makes a PCV1 vs PCV2 plot; will run smart pca to get .evec file if the .evec file is not provided.
## Input1: read in the stem name of the data or the stem name of the .evec file.
## Input2: If you want the points ot be color coded, provide a hash reference with the name=>color of the samples in the data/evec file
## Input3: For the subroutine to rerun smartpca, if the stem_name.evev file already exists, then specify 1;
...
sub make_PCA_plot
{
	...
	if($rerun_smartpca == 1 || !-e "$stem_name.evec")
        {
                run_smartpca($stem_name);
        }
	
	## Convert .evec file and color codes into new input file for R
	open(IN,"$stem_name.evec") or die "Can't open $stem_name.evec; $!\n";
	my $header = <IN>;
	...
	while(my $line = <IN>)
	{
		...
		my $color = "black";
		my $pop = $study_name;
		$pop = $population_color_codes_and_sample_population_hash_ref{"$fid\__$iid"} if exists $population_color_codes_and_sample_population_hash_ref{"$fid\__$iid"};
		$color = $population_color_codes_and_sample_population_hash_ref{$pop} if exists $population_color_codes_and_sample_population_hash_ref{$pop};
		$pops{$pop} = $color;
		$max_pop_name_length = length $pop  if length $pop > $max_pop_name_length; ## need this to correctly set the legend
		
		if(exists $population_color_codes_and_sample_population_hash_ref{"$fid\__$iid"} && $population_color_codes_and_sample_population_hash_ref{"$fid\__$iid"} =~ /^$study_name$/)
		{
			#print "non ref $pop\n";
...
{	
	...
	print "\nRunning smartpca on $stem_name => $stem_name.evec\n" if $verbose > 0;
	print $LOG "\nRunning smartpca on $stem_name => $stem_name.evec\n" if $verbose > 0;

	make_binary_version($stem_name);
	make_non_binary_version($stem_name);
	$intermediate_files{"$stem_name.ped"} = 1;
	$intermediate_files{"$stem_name.map"} = 1;

	## make affection column 2, because smartpca will ignore anyone with a missing value
	open(OUT,">$stem_name.fam_temp");
	open(IN,"$stem_name.fam");
	while(my $line =<IN>)
	{
		chomp($line);
		$line =~ s/^\s+//;
		$line =~ s/([^\s]+\s+[^\s]+\s+[^\s]+\s+[^\s]+\s+[^\s]+)\s+[^\s]+/$1 2/;
		print OUT "$line\n";
	}
	close(OUT);
	close(IN);
	system("mv $stem_name.fam_temp $stem_name.fam");

	# make .par file
	open (PAR,">$stem_name.par");
	print PAR "genotypename: $stem_name.ped\n";
	print PAR "snpname: $stem_name.map\n";
	print PAR "indivname: $stem_name.fam\n";
	print PAR "evecoutname: $stem_name.evec\n";
	print PAR "evaloutname: $stem_name.eval\n";
	print PAR "numoutlieriter: 0\n";
	close PAR;
	$intermediate_files{"$stem_name.par"} = 1;
	$intermediate_files{"$stem_name.evec"} = 10;
	$intermediate_files{"$stem_name.eval"} = 1;
	
	## Run Eigen Strat
	my $temp = system("$SMARTPCA -p $stem_name.par > $stem_name.log");
	if($temp > 0)
	{
		die "ERROR!!! Smartpca failed.\n";
	}
	$intermediate_files{"$stem_name.log"} = 1;
	return "$stem_name.evec";
}	
...
sub get_unrelateds
{
	...
	$intermediate_files{"$stem_name\_IMUS"} = 3;
...
sub calculate_IBD_estimates
{
	...
	my $temp = system("$PLINK --noweb --bfile $stem_name $read_freq --genome --maf $MAF --geno $GENO --mind $MIND $plink_silent --out $new_stem_name");
	if($temp > 0)
	{
...
sub keep_samples
{
	...
	## Make an unrelated version of the data
	my $temp = system("$PLINK --allow-no-sex --noweb --geno $GENO --mind $MIND --keep $samples_to_keep --bfile $stem_name --indiv-sort 0 --make-bed $plink_silent --out $new_stem_name");
	if($temp > 0){die "ERROR!!! PLINK failed to calculate IBD estimates; check log file: $new_stem_name.log\n";}
...
sub set_paths
{
	...
	foreach my $name (keys %$hash_ref)
	{
		...
		elsif($name eq "SMARTPCA"){$SMARTPCA=$$hash_ref{$name} }
