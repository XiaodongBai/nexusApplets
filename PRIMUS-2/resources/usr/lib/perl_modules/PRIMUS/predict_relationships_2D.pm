...
sub get_relationship_likelihood_vectors
{
	...
	open(PROB_OUT,">$outfile");
	open(MZ_OUT,">$output_dir/mz_twins");
	...
	while(my $line = <IN>)
	{
		...
		if(sum(0,@density_vector) < 0.000001)
		{
			...
			elsif($k1 > .8 * $KDE_density_resolution)
			{
		...
		## If the intial_likelihood_cutoff is dropped low enough, the FS will overlabe with HAG (2nd degree), 
		...
		$possibility_counts{$num_possibilities} += 1;
