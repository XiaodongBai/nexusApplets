...
my $MAX_RUNTIME = 3600; # 43200 = 12 hours; 129,600 = 36 hours
my $MAX_NETWORKS_TO_RESOLVE = 3000;
my $MAX_GENERATIONS = 4;
...
sub reconstruct_pedigree
{
	...
	if(@all_possible_networks eq 0 && $MIN_LIKELIHOOD > 0.01)
	{
		if($MIN_LIKELIHOOD <= .1)
		{
...
sub reconstruct_network
{
	...
	foreach my $network_ref (@networks_unresolved)
	{
		foreach my $node_name (keys %$network_ref)
		{
			...
			if(keys %PC >= 2)
			{
				my $resolved = resolve_PC_trio($network_ref,$node_name);
				## If $resolved == -1 then an error occurred and should return
				if($resolved == -1)
				{
					if($verbose > 0)
					{
						#print "resolve_PC-FS_trio return -1\n";
					}
					return;
				}
			}
	...
	print "Entering Phase 1. # of possible pedigrees: ". @networks_unresolved ."\n" if $verbose > 0;
	print $LOG "Entering Phase 1. # of possible pedigrees: ". @networks_unresolved ."\n" if $verbose > 0;
	...
	for(my $i = 0; $i < @PC_rels - 1; $i++)
	{
		for(my $j = $i+1; $j < @PC_rels; $j++)
		{
			...
			{		
				...
				{	
					my @parents = $$network_ref{$node_name}->parents();
					print "$node_name already has two parents (@parents) can't addthese two\n" if $verbose > 1;
					print $LOG "$node_name already has two parents (@parents) can't addthese two\n" if $verbose > 1;
					return -1;
				}
...
sub get_genders
{
	...
	## Continue cycling through the names until all have been resolved
	my $loop_ctr = 0;
	while($continue)
	{	
		$loop_ctr++;
		...
		if($name_ctr > @names-1)
		{
			...
			if(@names_resolved == @names)
			{
				## YOUR DONE
				#print "Genders after:\n";
				#foreach my $node (keys %$gender_ref)
				#{
				#	print "$node $$gender_ref{$node}\n";
				#}
				#print "\n";
				return $gender_ref;
			}
			$name_ctr = 0;
			$randomly_assign_sex_to_two_unknown_parents = 1;
		}
	...
	foreach my $network_ref (@network_refs)
	{
		foreach my $node_name (keys %{$network_ref})
		{
			...
			foreach my $rel_name (keys %{$network_ref})
			{
				...
				{
					print OUT "$FID1\t$IID1\t$FID2\t$IID2\t$rel\n";
				}
