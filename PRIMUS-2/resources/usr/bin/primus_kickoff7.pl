...
my $internal_ref = 0;
my $alt_ref_stem = "";
my $keep_prePRIMUS_intermediate_files = 0;
my $no_PCA_plot = 0;
my @ref_pops;
my $rerun = 0;
...
sub run_prePRIMUS
{
	...
	my %paths = ("LIB",$lib_dir,"PLINK",$plink_path,"SMARTPCA",$smartpca_path);
...
sub run_PR
{
	...
	foreach my $net (sort {$a <=> $b} keys %networks)
	{
		...
		{	
			...
			#system("dx upload -r $network_dir --path $output_dir/");
			#print "pr: @PR_commands\n";
			
			#system ("echo \"here1\" > here1");
			#system ("dx upload here1 --path PRIMUS:/applets/");
			
			#system("dx-run-app-locally PRIMUS -i PR_commands=\"@PR_commands\" -i output_dir=$output_dir -i dataset_name=$dataset_name -i network_dir=$network_dir -i single_family=1");	
			#system("dx run PRIMUS -i PR_commands=\"@PR_commands\" -i output_dir=$output_dir -i dataset_name=$dataset_name -i network_dir=$network_dir -i single_family=1");	
			#system("dx-jobutil-new-job run_pedigree_reconstruction_on_a_single_family_network -i PR_commands=\"@PR_commands\" -i output_dir=$output_dir -i dataset_name=$dataset_name -i network_dir=$network_dir -i single_family=1");	
...
sub get_sample_info
{
	...
	while(my $line = <IN>)
	{
		...
		if($FID =~ /\*\*/ || $FID =~ /,/ || $FID =~ /__/)
		{
			...
			die "FIDs and IIDs cannot contain '**' or ',' or '__'\n";
		}
		if($IID =~ /\*\*/ || $IID =~ /,/ || $IID =~ /__/)
		{
		...
		$hash{"$FID\__$IID"} = @temp[$trait_col-1];
	}
